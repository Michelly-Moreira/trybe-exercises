...
1.  function anything (event) {
  event.preventDefault();
}
  HREF_LINK.addEventListener('click', anything)


2.  function anyCheckBox (event){
    event.preventDefault();
  }
  INPUT_CHECKBOX.addEventListener('click', anyCheckBox)



3.    function anyKeys(event) {
    const caractere = event.key;
    if(caractere !== 'a'){
    event.preventDefault();
    }
  }
  INPUT_TEXT.addEventListener('keypress', anyKeys);*/
